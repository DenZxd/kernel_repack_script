...
find_start_end() 
{
    ...
    pos4=`grep -P -a -b --only-matching '\x00\x00\x00\x89\x4C\x5A\x4F' $zImage |
	 cut -f 1 -d : 2>/dev/null | awk '(int($0)<50000){print $0+3;exit}'`
...
NOSTDINC_FLAGS="-nostdinc -isystem $COMPILER_LIB/include -Iarch/arm/include \
		...
		-Iarch/arm/mach-omap2/include -Iarch/arm/plat-omap/include -Iarch/arm/plat-samsung/include"
